...
export class PerformanceMonitor {
	...
	static compareBenchmarks(
		...
		const processingTimeReduction =
			((baseline.processingTime - optimized.processingTime) / baseline.processingTime) * 100;
		const memoryReduction =
			((baseline.memoryUsage.heapUsed - optimized.memoryUsage.heapUsed) /
  ...
  Throughput: ${metrics.throughput.toFixed(2)} tokens/ms`);
	...
	static generateReport(benchmarks: PerformanceBenchmark[]): string {
		...
		benchmarks.forEach((benchmark, index) => {
			report.push(`\nBenchmark ${index + 1}:`);
			report.push(
				`  Processing Time: ${benchmark.baseline.processingTime.toFixed(2)}ms → ${benchmark.optimized.processingTime.toFixed(2)}ms (${benchmark.improvement.processingTimeReduction.toFixed(1)}% improvement)`
			);
			report.push(
				`  Memory Usage: ${(benchmark.baseline.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)}MB → ${(benchmark.optimized.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)}MB (${benchmark.improvement.memoryReduction.toFixed(1)}% improvement)`
			);
			report.push(
				`  Throughput: ${benchmark.baseline.throughput.toFixed(2)} → ${benchmark.optimized.throughput.toFixed(2)} tokens/ms (${benchmark.improvement.throughputIncrease.toFixed(1)}% improvement)`
			);
		});
