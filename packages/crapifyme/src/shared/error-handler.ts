...
export class ErrorHandler {
	...
	recordError(error: Partial<ParseError> & { category: ErrorCategory; message: string }): void {
		...
		switch (fullError.severity) {
			case ErrorSeverity.CRITICAL:
				this.logger.error(
					`CRITICAL ${fullError.category}: ${fullError.message}`,
	...
	private recoverFromStringError(
		...
		if (quote) {
			...
			if (nextQuote !== -1) {
				...
				result.warnings.push(
					`Recovered unterminated string by finding next ${quote} at position ${nextQuote}`
				);
				return result;
			}
		}
		...
		if (nextNewline !== -1) {
			result.recovered = true;
			result.warnings.push(
				`Recovered unterminated string by ending at line break at position ${nextNewline}`
			);
			return result;
		}
	...
	private recoverFromTemplateError(
		...
		if (nextBacktick !== -1) {
			result.recovered = true;
			result.warnings.push(
				`Recovered unterminated template literal by finding next backtick at position ${nextBacktick}`
			);
			return result;
		}
		...
		if (nextQuote !== -1) {
			result.recovered = true;
			result.warnings.push(
				`Recovered template literal by treating as string ending at position ${nextQuote}`
			);
			return result;
		}
	...
	private recoverFromRegexError(
		...
		while (i < content.length) {
			if (content[i] === '/' && (i === 0 || content[i - 1] !== '\\')) {
				result.recovered = true;
				result.warnings.push(
					`Recovered unterminated regex by finding next unescaped / at position ${i}`
				);
				return result;
			}
			i++;
	...
	private recoverFromCommentError(
		...
		if (content.substring(position, position + 2) === '/*') {
			...
			if (endComment !== -1) {
				result.recovered = true;
				result.warnings.push(
					`Recovered unterminated block comment by finding */ at position ${endComment}`
				);
				return result;
			}
		}
		...
		if (content.substring(position, position + 2) === '//') {
			...
			if (nextNewline !== -1) {
				result.recovered = true;
				result.warnings.push(
					`Recovered line comment by finding newline at position ${nextNewline}`
				);
				return result;
			}
		}
		...
		if (content.substring(position, position + 4) === '<!--') {
			...
			if (endComment !== -1) {
				result.recovered = true;
				result.warnings.push(
					`Recovered unterminated HTML comment by finding --> at position ${endComment}`
				);
				return result;
			}
		}
