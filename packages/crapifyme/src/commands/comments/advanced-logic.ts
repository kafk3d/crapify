...
export class AdvancedCommentRemover {
	...
	removeComments(content: string, filePath: string): ProcessResult {
		...
		try {
			...
			this.logger.warn(
				`Enhanced tokenizer failed for ${filePath}, falling back to legacy tokenizer`
			);
	...
	private isPartOfUrl(content: string, position: number): boolean {
	...
	private configurePreservationRules(options: AdvancedCommentsOptions): void {
	...
	private enhanceResultWithErrorInfo(result: ProcessResult, filePath: string): ProcessResult {
		...
		if (allErrors.length > 0) {
			...
			this.logger.warn(
				`File ${filePath} processed with ${errorSummary.total} errors: ${JSON.stringify(errorSummary.bySeverity)}`
			);
		}
	...
	private getErrorSummaryDetailed(errors: any[]): {
		total: number;
		bySeverity: Record<string, number>;
		byCategory: Record<string, number>;
	} {
