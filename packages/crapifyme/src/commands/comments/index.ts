...
export const commentsCommand = new Command('comments')
    ...
    .option('-e, --extensions <ext>', 'File extensions to process', 'js,ts,jsx,tsx,vue,svelte,astro,html,css,scss,less,sass,md,mdx')
    ...
    .action(async (paths: string[], options: any, command: Command) => {
        ...
        try {
            ...
            const excludePatterns = options.exclude?.split(',').map((p: string) => p.trim()).filter(Boolean) || [];
            ...
            const exitCode = stats.errors.length > 0 ? ExitCode.Error : 
                           stats.commentsRemoved > 0 ? ExitCode.IssuesFound : ExitCode.Success;
